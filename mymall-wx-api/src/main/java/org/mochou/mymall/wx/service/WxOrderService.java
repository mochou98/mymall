...
import org.mochou.mymall.core.express.ExpressService;
...
import org.mochou.mymall.core.util.DateTimeUtil;
...
import org.mochou.mymall.db.util.OrderHandleOption;
...
public class WxOrderService {
    ...
    private String detailedAddress(MymallAddress mymallAddress) {
        Integer provinceId = mymallAddress.getProvinceId();
        Integer cityId = mymallAddress.getCityId();
        Integer areaId = mymallAddress.getAreaId();
        ...
        return fullRegion + " " + mymallAddress.getAddress();
    }
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer size) {
        ...
        List<MymallOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, size);
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            MymallGrouponRules rules = grouponRulesService.queryById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
        ...
        // 收货地址
        MymallAddress checkedAddress = addressService.findById(addressId);
        ...
        // 如果使用了优惠券，设置优惠券使用状态
        if (couponId != 0 && couponId != -1) {
            MymallCouponUser couponUser = couponUserService.queryOne(userId, couponId);
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        MymallGroupon groupon = grouponService.queryByOrderId(order.getId());
        if (groupon != null) {
            MymallGrouponRules grouponRules = grouponRulesService.queryById(groupon.getRulesId());
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<MymallOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        MymallOrderGoods orderGoods = orderGoodsList.get(0);
